...
class Dataviz_AI_Digest_Mailer {
	...
	public static function send_html( $to, $subject, $html, array $headers = array() ) {
		...
		$msg = $captured
			? $captured
			: __( 'The server could not send email. On local or Docker installs, PHP mail() is usually disabled — install an SMTP plugin (e.g. WP Mail SMTP) or configure your host’s mail.', 'dataviz-ai-for-woocommerce' );

		// PHPMailer when PHP mail() is missing/broken (typical on Docker/local).
		if ( is_string( $msg ) && stripos( $msg, 'instantiate mail' ) !== false ) {
			$msg .= "\n\n" . __( 'What this means: WordPress tried to use PHP’s mail() function, but this server cannot run it.', 'dataviz-ai-for-woocommerce' )
				. ' ' . __( 'Fix: Install “WP Mail SMTP” (free), choose “Other SMTP”, and enter Gmail (app password), Mailtrap, SendGrid, or your host’s SMTP. Then digests will send like any other WordPress email.', 'dataviz-ai-for-woocommerce' );
		}
