...
class Dataviz_AI_Data_Fetcher {
	...
	public function get_recent_orders( array $args = array() ) {
		...
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] get_recent_orders called with args: %s', wp_json_encode( $args ) ) );
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] get_recent_orders query_args: %s', wp_json_encode( $query_args ) ) );

		$orders = wc_get_orders( $query_args );
		
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] get_recent_orders returned %d orders', count( $orders ) ) );
	...
	public function get_order_statistics( array $args = array() ) {
		...
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] get_order_statistics called with args: %s', wp_json_encode( $args ) ) );
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] Found %d orders', count( $orders ) ) );
	...
	public function get_customer_statistics( array $args = array() ) {
		...
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] get_customer_statistics called with args: %s', wp_json_encode( $args ) ) );
		dataviz_ai_wc_debug_log( sprintf( '[Store Compass] Found %d orders for customer statistics', count( $orders ) ) );
	...
	public function get_all_inventory_products( array $filters = array() ) {
		if ( ! function_exists( 'wc_get_products' ) ) {
			return array(
				'error'   => true,
				'message' => __( 'WooCommerce is not available. Please ensure WooCommerce is installed and activated.', 'store-compass-for-woocommerce' ),
			);
		}

		$raw_limit = isset( $filters['limit'] ) ? (int) $filters['limit'] : 100;
		$limit     = ( $raw_limit === -1 ) ? -1 : min( 500, max( 1, $raw_limit ) );
		...
		return array(
			...
			'message'  => sprintf(
				...
				_n( 'Found %d product with inventory information.', 'Found %d products with inventory information.', count( $inventory ), 'store-compass-for-woocommerce' ),
				count( $inventory )
			),
		);
	}
