...
class Dataviz_AI_Query_Orchestrator {
	...
	public function handle_stream( $question ) {
		...
		if ( $pipeline_result['error'] instanceof \WP_Error ) {
			...
			$this->stream_handler->send_error(
				__( 'Unable to process data query. Please try rephrasing your question.', 'store-compass-for-woocommerce' )
			);
			return;
		}
	...
	public function handle_non_stream( $question ) {
		...
		if ( $this->is_feature_request_confirmation( $question ) ) {
			...
			if ( $entity_type ) {
				...
				return array( 'answer' => __( 'Failed to submit feature request. Please try again later.', 'store-compass-for-woocommerce' ), 'provider' => 'system' );
			}
		}
		...
		if ( $answer === '' ) {
			return new \WP_Error( 'dataviz_ai_invalid_response', __( 'Unexpected response format from AI.', 'store-compass-for-woocommerce' ) );
		}
	...
	protected function handle_custom_backend_stream( $question ) {
		...
		$answer = isset( $response['answer'] ) ? $response['answer'] : __( 'Response received.', 'store-compass-for-woocommerce' );
	...
	protected function build_intent_not_found_response( $question, $reason = '', array $intent_snapshot = array(), array $options = array() ) {
		...
		if ( $entity_type === 'comparisons' ) {
			$message = __( 'Comparisons across periods (for example, January vs February) are not currently supported in this version.', 'store-compass-for-woocommerce' );
		} elseif ( $entity_type === 'conversion_rate' ) {
			$message = __( 'Conversion rate is not currently supported because this store is not connected to traffic/session analytics data yet.', 'store-compass-for-woocommerce' );
		} elseif ( $low_confidence ) {
			$message = __( 'I am not fully confident I understood that question, so I did not run a WooCommerce data query yet.', 'store-compass-for-woocommerce' );
		} else {
			$message = __( 'I was not able to understand this request well enough to fetch WooCommerce data for it yet.', 'store-compass-for-woocommerce' );
		}
		$feature_prompt = __( 'Would you like to request this feature? Just say "yes" and I will submit a feature request to the administrators so we can support questions like this.', 'store-compass-for-woocommerce' );
		$clarify_prompt = __( 'Please try again with one clear target and timeframe, for example: "Show pending orders for this week" or "List out-of-stock products".', 'store-compass-for-woocommerce' );
		...
		if ( $examples !== '' ) {
			$answer .= "\n\n" . __( 'Examples you can try:', 'store-compass-for-woocommerce' ) . "\n" . $examples;
		}
