...
class Dataviz_AI_Answer_Composer {
	...
	private static function compose_from_single_tool_result( $question, array $validated_intent, array $single ) {
			...
			if ( is_array( $res ) && isset( $res['error'] ) && $res['error'] === true && ( $res['can_submit_request'] ?? false ) === true ) {
				$message = isset( $res['message'] ) ? (string) $res['message'] : __( 'This request is not currently supported.', 'unmai-analytix-for-woocommerce' );
			...
			if ( $wants_out_of_stock && is_array( $res ) && ( empty( $res ) || isset( $res[0]['id'] ) ) ) {
				if ( empty( $res ) ) {
					return __( 'There are no out-of-stock products.', 'unmai-analytix-for-woocommerce' );
				}
				...
				foreach ( $res as $idx => $p ) {
					...
					$name = isset( $p['name'] ) ? (string) $p['name'] : __( '(Unnamed product)', 'unmai-analytix-for-woocommerce' );
					...
					$qty_text = $manage_stock ? ( $qty === null ? '0' : (string) $qty ) : __( 'N/A', 'unmai-analytix-for-woocommerce' );
					$sku_text = $sku !== '' ? ' (SKU: ' . $sku . ')' : '';

					$lines[] = sprintf( '%d. %s%s — Qty: %s', (int) ( $idx + 1 ), $name, $sku_text, $qty_text );
				}

				return __( 'Out-of-stock products:', 'unmai-analytix-for-woocommerce' ) . "\n" . implode( "\n", $lines );
			}
			...
			if ( is_array( $res ) && ! isset( $res['error'] ) && isset( $res['orders'] ) && is_array( $res['orders'] ) && empty( $res['orders'] ) ) {
				...
				if ( isset( $args['filters']['date_from'], $args['filters']['date_to'] ) ) {
					$period_text = ' ' . sprintf(
						...
						__( 'in the period from %1$s to %2$s', 'unmai-analytix-for-woocommerce' ),
					...
					$period_text = ' ' . __( 'for the requested period', 'unmai-analytix-for-woocommerce' );
				}
				return sprintf(
					/* translators: %s: optional date-range phrase (may be empty) */
					__( 'There are no orders%1$s.', 'unmai-analytix-for-woocommerce' ),
					$period_text
				);
			}
			...
			if ( $is_customer_stats && is_array( $res ) && ! isset( $res['error'] ) && isset( $res['customers'] ) && is_array( $res['customers'] ) ) {
				if ( empty( $res['customers'] ) ) {
					...
					if ( $is_how_many ) {
						return sprintf(
							/* translators: %s: optional date-range phrase (may be empty) */
							__( '0 customers have placed orders%1$s.', 'unmai-analytix-for-woocommerce' ),
							$period_text
						);
					}
					return sprintf(
						/* translators: %s: optional date-range phrase (may be empty) */
						__( 'No customers found with orders%1$s.', 'unmai-analytix-for-woocommerce' ),
						$period_text
					);
				}
				...
				foreach ( $res['customers'] as $idx => $c ) {
					...
					if ( $name === '' ) {
						$name = isset( $c['username'] ) ? (string) $c['username'] : __( '(Unknown customer)', 'unmai-analytix-for-woocommerce' );
					}
				...
				$header = __( 'Top customers by total spend:', 'unmai-analytix-for-woocommerce' );
				return $header . "\n" . implode( "\n", $lines );
			}
				...
				if ( $tag_query !== '' ) {
					...
					if ( is_array( $tag ) ) {
						...
						return sprintf(
							...
							__( 'There are %1$d products with the tag "%2$s".', 'unmai-analytix-for-woocommerce' ),
							$count,
							$tag['name'] ?? $tag_query
						);
					}
					return sprintf(
						...
						__( 'I couldn’t find a product tag named "%1$s".', 'unmai-analytix-for-woocommerce' ),
						$tag_query
					);
				}
			}
			...
			if ( $is_coupon_usage && is_array( $res ) && ! isset( $res['error'] ) && isset( $res['coupons'] ) && is_array( $res['coupons'] ) ) {
				...
				if ( empty( $res['coupons'] ) ) {
					return $period
						? sprintf(
							...
							__( 'No coupons were used %1$s.', 'unmai-analytix-for-woocommerce' ),
							$period
						)
						: __( 'No coupons were used in the requested period.', 'unmai-analytix-for-woocommerce' );
				}
				...
				$header = $period
					? sprintf(
						/* translators: %s: date-range phrase */
						__( 'Coupons used %1$s:', 'unmai-analytix-for-woocommerce' ),
						$period
					)
					: __( 'Coupons used in the requested period:', 'unmai-analytix-for-woocommerce' );
				return $header . "\n" . implode( "\n", $lines );
			}
			...
			if ( $is_refunds && is_array( $res ) ) {
				...
				if ( empty( $res ) ) {
					...
					if ( $is_how_many_refunds ) {
						return sprintf(
							/* translators: %s: optional date-range phrase (may be empty) */
							__( 'There are 0 refunds%1$s.', 'unmai-analytix-for-woocommerce' ),
							$period_text
						);
					}
					return sprintf(
						/* translators: %s: optional date-range phrase (may be empty) */
						__( 'No refunds found%1$s.', 'unmai-analytix-for-woocommerce' ),
						$period_text
					);
				}
				...
				if ( $is_how_many_refunds ) {
					...
					return sprintf(
						...
						__( 'There are %1$d refunds%2$s, totaling %3$s.', 'unmai-analytix-for-woocommerce' ),
						$count,
						$period_text,
						$amount_str
					);
				...
				foreach ( $res as $idx => $r ) {
					...
					$reason = isset( $r['reason'] ) && $r['reason'] !== '' ? (string) $r['reason'] : __( 'No reason given', 'unmai-analytix-for-woocommerce' );
					...
					if ( $reason !== __( 'No reason given', 'unmai-analytix-for-woocommerce' ) ) {
						$line .= ' — ' . $reason;
					}
					if ( $date !== '' ) {
						$line .= ' — ' . $date;
				...
				$header = sprintf(
					...
					__( '%1$d refunds found:', 'unmai-analytix-for-woocommerce' ),
					$count
				);
				return $header . "\n" . implode( "\n", $lines );
			}
			...
			if ( $is_order_stats && is_array( $res ) && ! isset( $res['error'] ) && isset( $res['summary'] ) && is_array( $res['summary'] ) && array_key_exists( 'total_revenue', $res['summary'] ) ) {
				...
				if ( $asks_count && ( $status_filter || $asks_specific_status ) ) {
					...
					if ( 0 === $count ) {
						return sprintf(
							...
							__( 'There are currently no %1$s orders.', 'unmai-analytix-for-woocommerce' ),
					...
					if ( null !== $rev_for_status && $rev_for_status > 0 ) {
						$answer .= ' ' . sprintf(
							...
							__( 'Their total value is %1$s.', 'unmai-analytix-for-woocommerce' ),
				...
				if ( $wants_sales_by_category ) {
					if ( empty( $category_breakdown ) || ! is_array( $category_breakdown ) ) {
						...
						if ( isset( $res['date_range'] ) && is_array( $res['date_range'] ) ) {
							...
							if ( $df && $dt ) {
								$period_text = ' ' . sprintf(
									...
									__( 'in the selected period (%1$s to %2$s)', 'unmai-analytix-for-woocommerce' ),
									$df,
									$dt
								);
							}
						...
						return sprintf(
							/* translators: %s: optional date-range phrase (may be empty) */
							__( 'There are no sales records by product category%1$s. This can happen when there are no completed orders, or when ordered products are not assigned to categories.', 'unmai-analytix-for-woocommerce' ),
							$period_text
						);
					}
					// Non-empty category_breakdown: let LLM summarize; frontend will render chart from tool data.
					return null;
				}
				...
				$answer = sprintf(
					...
					__( 'The total revenue generated%1$s is %2$s.', 'unmai-analytix-for-woocommerce' ),
				...
				if ( null !== $total_orders ) {
						...
						if ( null !== $bd_c ) {
							...
							if ( 0 === $bd_c ) {
								$answer .= ' ' . sprintf(
									...
									__( 'There are no %1$s orders in this period.', 'unmai-analytix-for-woocommerce' ),
									$lbl
								);
							} elseif ( 1 === $bd_c ) {
								$answer .= ' ' . sprintf(
									/* translators: %s: order status label */
									__( 'There is 1 %1$s order in this period.', 'unmai-analytix-for-woocommerce' ),
									$lbl
								);
							} else {
								$answer .= ' ' . sprintf(
									/* translators: 1: order count, 2: order status label */
									__( 'There are %1$d %2$s orders in this period.', 'unmai-analytix-for-woocommerce' ),
									$bd_c,
									$lbl
								);
							}
					...
					if ( ! $used_status_line ) {
						if ( $total_orders === 0 ) {
							$answer .= ' ' . __( 'There are no orders in this period.', 'unmai-analytix-for-woocommerce' );
						} elseif ( 1 === $total_orders ) {
							$answer .= ' ' . __( 'There is 1 order in this period.', 'unmai-analytix-for-woocommerce' );
						} else {
							$answer .= ' ' . sprintf(
								/* translators: %d: order count */
								__( 'There are %1$d orders in this period.', 'unmai-analytix-for-woocommerce' ),
								$total_orders
							);
						}
					}
				...
				if ( $wants_status && ! empty( $res['status_breakdown'] ) && is_array( $res['status_breakdown'] ) ) {
					...
					foreach ( $res['status_breakdown'] as $sb ) {
						...
						$s_name  = isset( $sb['status'] ) ? ucfirst( (string) $sb['status'] ) : __( 'Unknown', 'unmai-analytix-for-woocommerce' );
					...
					if ( ! empty( $status_lines ) ) {
						$answer .= "\n\n" . __( 'Breakdown by status:', 'unmai-analytix-for-woocommerce' ) . "\n" . implode( "\n", $status_lines );
					}
				}
	...
	private static function compose_orders_list_answer( array $orders, $shown, $total ) {
		...
		foreach ( $orders as $idx => $o ) {
			...
			$lines[]    = sprintf(
				'%d. %s — %s — %s — %s',
				(int) ( $idx + 1 ),
				sprintf( /* translators: %d: order ID */ __( 'Order #%1$d', 'unmai-analytix-for-woocommerce' ), $id ),
		...
		if ( $total > $shown ) {
			$header = sprintf(
				...
				__( 'Showing %1$d of %2$d orders (newest first):', 'unmai-analytix-for-woocommerce' ),
				$shown,
				$total
			);
		} else {
			$header = sprintf(
				/* translators: %d order count */
				_n( '%d order:', '%d orders:', $total, 'unmai-analytix-for-woocommerce' ),
				$total
			);
		}
		return $header . "\n" . implode( "\n", $lines );
