...
class Dataviz_AI_Digest_Cron {
	...
	public function add_cron_interval( $schedules ) {
		if ( ! isset( $schedules[ self::CRON_INTERVAL ] ) ) {
			$schedules[ self::CRON_INTERVAL ] = array(
				...
				'display'  => __( 'Every 15 minutes', 'unmai-analytix-for-woocommerce' ),
			);
		}
		return $schedules;
	}
	...
	private function send_digest( $digest ) {
		...
		$subject = sprintf(
			...
			__( '[%2$s] %1$s', 'unmai-analytix-for-woocommerce' ),
			$digest->digest_name,
			get_bloginfo( 'name' )
		);
	...
	private function log( $message ) {
		dataviz_ai_wc_debug_log( '[Unmai Analytix Digest] ' . $message );
	}
}
