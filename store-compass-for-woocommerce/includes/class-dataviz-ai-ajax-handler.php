<?php
/**
 * Thin AJAX endpoint handler for Unmai Analytix WooCommerce plugin.
 *
...
class Dataviz_AI_AJAX_Handler {
	...
	public function handle_analysis_request() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Unauthorized request.', 'unmai-analytix-for-woocommerce' ) ), 403 );
		}

		$question = isset( $_POST['question'] ) ? sanitize_text_field( wp_unslash( $_POST['question'] ) ) : __( 'Provide a quick performance summary.', 'unmai-analytix-for-woocommerce' );
		$stream = filter_var(
	...
	public function handle_chat_request() {
		...
		if ( empty( $question ) ) {
			wp_send_json_error( array( 'message' => __( 'Message cannot be empty.', 'unmai-analytix-for-woocommerce' ) ), 400 );
		}
		...
		if ( empty( $content ) ) {
			wp_send_json_error( array( 'message' => __( 'The AI response was empty. Try again.', 'unmai-analytix-for-woocommerce' ), 'data' => $result ), 400 );
		}
	...
	public function handle_submit_feature_request() {
		...
		if ( empty( $entity_type ) ) {
			wp_send_json_error( array( 'message' => __( 'Entity type is required.', 'unmai-analytix-for-woocommerce' ) ), 400 );
		}
		...
		if ( $request_id ) {
			wp_send_json_success( array(
				'message'    => sprintf(
					...
					__( 'Feature request for "%1$s" has been submitted successfully! Request ID: #%2$d.', 'unmai-analytix-for-woocommerce' ),
		...
		wp_send_json_error( array( 'message' => __( 'Failed to submit feature request. Please try again later.', 'unmai-analytix-for-woocommerce' ) ), 500 );
	}
	...
	public function handle_debug_intent_request() {
		...
		if ( ! defined( 'DATAVIZ_AI_DEBUG_INTENT' ) || ! DATAVIZ_AI_DEBUG_INTENT ) {
			wp_send_json_error( array( 'message' => __( 'Debug intent is disabled.', 'unmai-analytix-for-woocommerce' ) ), 403 );
		}
		...
		if ( $question === '' ) {
			wp_send_json_error( array( 'message' => __( 'Question is required.', 'unmai-analytix-for-woocommerce' ) ), 400 );
		}
	...
	protected function build_openai_messages( $question, $orders ) {
		...
		foreach ( $orders as $order ) {
			...
			$orders_summary[] = sprintf(
				...
				$order->get_date_created() ? $order->get_date_created()->date_i18n( 'Y-m-d' ) : __( 'N/A', 'unmai-analytix-for-woocommerce' ),
				count( $order->get_items() )
			);
		}
		$context_block = $orders_summary ? implode( "\n", $orders_summary ) : __( 'No recent orders available.', 'unmai-analytix-for-woocommerce' );

		return array(
			array( 'role' => 'system', 'content' => __( 'You are a helpful WooCommerce analytics assistant. Answer clearly and concisely based on the provided store data.', 'unmai-analytix-for-woocommerce' ) ),
