...
class Dataviz_AI_Digest_Admin {
	...
	public function register_submenu() {
		add_submenu_page(
			...
			__( 'Email Digests', 'dataviz-ai-for-woocommerce' ),
			__( 'Email Digests', 'dataviz-ai-for-woocommerce' ),
			'manage_woocommerce',
			'dataviz-ai-digests',
	...
	private function handle_save() {
		// phpcs:disable WordPress.Security.NonceVerification.Missing -- Nonce verified in process_actions() before this method runs.
		$sections_raw = array();
		if ( isset( $_POST['sections'] ) && is_array( $_POST['sections'] ) ) {
			$sections_raw = array_map( 'sanitize_text_field', wp_unslash( $_POST['sections'] ) );
		}
		$data         = array(
			'digest_name'  => isset( $_POST['digest_name'] ) ? sanitize_text_field( wp_unslash( $_POST['digest_name'] ) ) : '',
			...
			'day_of_month' => isset( $_POST['day_of_month'] ) ? absint( wp_unslash( $_POST['day_of_month'] ) ) : 1,
			'send_hour'    => isset( $_POST['send_hour'] ) ? absint( wp_unslash( $_POST['send_hour'] ) ) : 9,
			'sections'     => $sections_raw,
			'recipients'   => array_filter(
				array_map(
					'sanitize_email',
					explode( ',', isset( $_POST['recipients'] ) ? sanitize_text_field( wp_unslash( $_POST['recipients'] ) ) : '' )
				)
			),
		);

		$id = isset( $_POST['digest_id'] ) ? absint( wp_unslash( $_POST['digest_id'] ) ) : 0;
	...
	private function handle_send_now( $id ) {
		...
		foreach ( $recipients as $email ) {
			...
			if ( true === $result ) {
				...
				$msg = is_wp_error( $result ) ? $result->get_error_message() : __( 'Unknown error', 'dataviz-ai-for-woocommerce' );
				$errors[] = $email . ': ' . $msg;
			}
		}
	...
	private function render_list() {
		...
		<div class="wrap dataviz-digest-wrap">
			...
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Email Digests', 'dataviz-ai-for-woocommerce' ); ?></h1>
			<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'dataviz-ai-digests', 'digest_action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New Digest', 'dataviz-ai-for-woocommerce' ); ?></a>
			...
			<?php if ( empty( $digests ) ) : ?>
				<div class="dataviz-digest-empty">
					...
					<h2><?php esc_html_e( 'No digests configured yet', 'dataviz-ai-for-woocommerce' ); ?></h2>
					<p><?php esc_html_e( 'Schedule automated email reports to stay on top of your store performance without logging in.', 'dataviz-ai-for-woocommerce' ); ?></p>
					<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'dataviz-ai-digests', 'digest_action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>" class="button button-primary button-hero"><?php esc_html_e( 'Create Your First Digest', 'dataviz-ai-for-woocommerce' ); ?></a>
				</div>
			<?php else : ?>
				<table class="wp-list-table widefat fixed striped dataviz-digest-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Name', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Frequency', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Schedule', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Sections', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Status', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Last Sent', 'dataviz-ai-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Next Run', 'dataviz-ai-for-woocommerce' ); ?></th>
					</tr>
				</thead>
				<tbody>
				<?php foreach ( $digests as $d ) : ?>
					...
					$toggle_label  = $d->status === 'active' ? __( 'Pause', 'dataviz-ai-for-woocommerce' ) : __( 'Activate', 'dataviz-ai-for-woocommerce' );
					...
					if ( $d->frequency === 'daily' ) {
						...
						$schedule_label = sprintf( __( 'Daily at %1$s', 'dataviz-ai-for-woocommerce' ), $send_time );
					} elseif ( $d->frequency === 'weekly' ) {
						/* translators: 1: weekday name, 2: send time in 24-hour format */
						$schedule_label = sprintf( __( '%1$s at %2$s', 'dataviz-ai-for-woocommerce' ), $days[ $d->day_of_week % 7 ], $send_time );
					} else {
						/* translators: 1: day of month (1–31), 2: send time in 24-hour format */
						$schedule_label = sprintf( __( 'Day %1$d at %2$s', 'dataviz-ai-for-woocommerce' ), (int) $d->day_of_month, $send_time );
					}
					...
					<tr>
						<td>
							...
							<div class="row-actions">
								<span><a href="<?php echo esc_url( $edit_url ); ?>"><?php esc_html_e( 'Edit', 'dataviz-ai-for-woocommerce' ); ?></a> | </span>
								<span><a href="<?php echo esc_url( $preview_url ); ?>"><?php esc_html_e( 'Preview', 'dataviz-ai-for-woocommerce' ); ?></a> | </span>
								<span><a href="<?php echo esc_url( $send_now_url ); ?>"><?php esc_html_e( 'Send Now', 'dataviz-ai-for-woocommerce' ); ?></a> | </span>
								<span><a href="<?php echo esc_url( $toggle_url ); ?>"><?php echo esc_html( $toggle_label ); ?></a> | </span>
								<span class="delete"><a href="<?php echo esc_url( $delete_url ); ?>" onclick="return confirm('<?php esc_attr_e( 'Delete this digest?', 'dataviz-ai-for-woocommerce' ); ?>');"><?php esc_html_e( 'Delete', 'dataviz-ai-for-woocommerce' ); ?></a></span>
							</div>
						</td>
						...
						<td>
							<?php if ( $d->status === 'active' ) : ?>
								<span class="dataviz-digest-status dataviz-digest-status--active"><?php esc_html_e( 'Active', 'dataviz-ai-for-woocommerce' ); ?></span>
							<?php else : ?>
								<span class="dataviz-digest-status dataviz-digest-status--paused"><?php esc_html_e( 'Paused', 'dataviz-ai-for-woocommerce' ); ?></span>
							<?php endif; ?>
						</td>
	...
	private function render_form() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only form display; digest_id sanitized.
		$id     = isset( $_GET['digest_id'] ) ? absint( wp_unslash( $_GET['digest_id'] ) ) : 0;
		$digest = $id > 0 ? Dataviz_AI_Email_Digests::get( $id ) : null;

		$name       = $digest ? $digest->digest_name : __( 'Weekly Sales Digest', 'dataviz-ai-for-woocommerce' );
		...
		?>
		<div class="wrap dataviz-digest-wrap">
			<h1><?php echo $is_edit ? esc_html__( 'Edit Digest', 'dataviz-ai-for-woocommerce' ) : esc_html__( 'New Digest', 'dataviz-ai-for-woocommerce' ); ?></h1>
			<hr class="wp-header-end">
			...
			<form method="post" class="dataviz-digest-form">
				...
				<tr>
					<th scope="row"><label for="digest_name"><?php esc_html_e( 'Digest Name', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td><input type="text" id="digest_name" name="digest_name" value="<?php echo esc_attr( $name ); ?>" class="regular-text" required></td>
				</tr>

				<tr>
					<th scope="row"><label for="frequency"><?php esc_html_e( 'Frequency', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td>
				...
				<tr class="dataviz-digest-row--weekly" <?php echo $frequency !== 'weekly' ? 'style="display:none"' : ''; ?>>
					<th scope="row"><label for="day_of_week"><?php esc_html_e( 'Day of Week', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td>
				...
				<tr class="dataviz-digest-row--monthly" <?php echo $frequency !== 'monthly' ? 'style="display:none"' : ''; ?>>
					<th scope="row"><label for="day_of_month"><?php esc_html_e( 'Day of Month', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Capped at 28 to work for all months.', 'dataviz-ai-for-woocommerce' ); ?></p>
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="send_hour"><?php esc_html_e( 'Send Time', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Server timezone.', 'dataviz-ai-for-woocommerce' ); ?></p>
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="recipients"><?php esc_html_e( 'Recipients', 'dataviz-ai-for-woocommerce' ); ?></label></th>
					<td>
						<input type="text" id="recipients" name="recipients" value="<?php echo esc_attr( $recipients ); ?>" class="large-text">
						<p class="description"><?php esc_html_e( 'Comma-separated email addresses. The creating user is always included.', 'dataviz-ai-for-woocommerce' ); ?></p>
					</td>
				</tr>

				<tr>
					<th scope="row"><?php esc_html_e( 'Report Sections', 'dataviz-ai-for-woocommerce' ); ?></th>
					<td>
						<fieldset>
				...
				<?php submit_button( $is_edit ? __( 'Update Digest', 'dataviz-ai-for-woocommerce' ) : __( 'Create Digest', 'dataviz-ai-for-woocommerce' ) ); ?>
			</form>
	...
	private function render_preview() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Preview view; digest_id sanitized.
		$id     = isset( $_GET['digest_id'] ) ? absint( wp_unslash( $_GET['digest_id'] ) ) : 0;
		$digest = $id > 0 ? Dataviz_AI_Email_Digests::get( $id ) : null;

		if ( ! $digest ) {
			echo '<div class="wrap"><div class="notice notice-error"><p>' . esc_html__( 'Digest not found.', 'dataviz-ai-for-woocommerce' ) . '</p></div></div>';
			return;
		}
		...
		<div class="wrap dataviz-digest-wrap">
			<h1>
				<?php esc_html_e( 'Email Preview', 'dataviz-ai-for-woocommerce' ); ?>
				<a href="<?php echo esc_url( $back_url ); ?>" class="page-title-action"><?php esc_html_e( 'Back to Digests', 'dataviz-ai-for-woocommerce' ); ?></a>
			</h1>
			<hr class="wp-header-end">
	...
	private function render_notices() {
		...
		$messages = array(
			'created'       => __( 'Digest created successfully.', 'dataviz-ai-for-woocommerce' ),
			'updated'       => __( 'Digest updated successfully.', 'dataviz-ai-for-woocommerce' ),
			'deleted'       => __( 'Digest deleted.', 'dataviz-ai-for-woocommerce' ),
			'paused'        => __( 'Digest paused.', 'dataviz-ai-for-woocommerce' ),
			'activated'     => __( 'Digest activated.', 'dataviz-ai-for-woocommerce' ),
			'sent'          => __( 'Digest sent successfully.', 'dataviz-ai-for-woocommerce' ),
			'send_partial'  => __( 'Digest partially sent — some addresses failed (see details below).', 'dataviz-ai-for-woocommerce' ),
			'send_failed'   => __( 'Email was not sent. Your server may not be configured to send mail (common on local/Docker).', 'dataviz-ai-for-woocommerce' ),
			'no_recipients' => __( 'No valid recipients found.', 'dataviz-ai-for-woocommerce' ),
			'not_found'     => __( 'Digest not found.', 'dataviz-ai-for-woocommerce' ),
		);
		...
		if ( in_array( $notice, array( 'send_failed', 'send_partial' ), true ) ) {
			...
			if ( $detail ) {
				...
				printf(
					...
					esc_html__( 'Details:', 'dataviz-ai-for-woocommerce' ),
					esc_html( $detail )
				);
			}
		}
	...
	private function render_email_delivery_help() {
		...
		<div class="notice notice-info inline" style="margin:12px 0 8px;padding:12px 14px;">
			<p style="margin:0 0 8px;"><strong><?php esc_html_e( 'Not receiving digest emails?', 'dataviz-ai-for-woocommerce' ); ?></strong></p>
			<ul style="margin:0 0 0 1.2em;list-style:disc;">
				<li><?php esc_html_e( 'WordPress uses PHP mail() by default. Local Docker/Vagrant and many hosts do not deliver it — install an SMTP plugin (e.g. WP Mail SMTP) and use Mailtrap, Gmail SMTP, or your host’s SMTP.', 'dataviz-ai-for-woocommerce' ); ?></li>
				<li><?php esc_html_e( 'After clicking “Send Now”, check for an error notice above — we now show the real failure reason when available.', 'dataviz-ai-for-woocommerce' ); ?></li>
				<li><?php esc_html_e( 'Scheduled sends rely on WP-Cron (triggered by site visits). Low-traffic sites may need a system cron hitting wp-cron.php.', 'dataviz-ai-for-woocommerce' ); ?></li>
			</ul>
		</div>
		<?php
	}
