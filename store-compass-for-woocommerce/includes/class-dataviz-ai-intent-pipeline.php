...
class Dataviz_AI_Intent_Pipeline {
	...
	public function process( $question ) {
		...
		if ( isset( $validated['confidence'] ) && $validated['confidence'] === 'low' ) {
			$result['low_confidence']  = true;
			$result['error_reason']   = __( 'Parsed intent confidence is low; no WooCommerce data query was run.', 'unmai-analytix-for-woocommerce' );
	...
	protected function feature_request_reason( $entity ) {
		...
		switch ( $entity ) {
			case 'comparisons':
				return __( 'Comparison queries are not currently supported.', 'unmai-analytix-for-woocommerce' );
			case 'conversion_rate':
				return __( 'Conversion-rate queries require traffic/session data, which is not currently available.', 'unmai-analytix-for-woocommerce' );
			case 'cross_entity_analysis':
				return __( 'Cross-entity analysis queries are not currently supported.', 'unmai-analytix-for-woocommerce' );
			default:
				return __( 'This capability is not currently supported.', 'unmai-analytix-for-woocommerce' );
		}
	}
}
