...
1. **Release / WordPress.org:** unzip so `wp-content/plugins/unmai-analytix-for-woocommerce/` contains the plugin (main file `dataviz-ai-woocommerce.php`). **From this repo:** copy `dataviz-ai-woocommerce-plugin/` into `wp-content/plugins/` and sync/rename to match your environment, or run `bash bin/sync-to-docker.sh` for Docker.
